    ...
    <main className="mx-auto max-w-[90%] my-8">
      <Card>
        ...
        <div className="relative rounded-lg font-semibold shadow-[0px_2px_3px_-1px_rgba(0,0,0,0.1),0px_1px_0px_0px_rgba(25,28,33,0.02),0px_0px_0px_1px_rgba(25,28,33,0.08)] grid grid-cols-2 text-2xl overflow-hidden">
          ...
          <button
            className={`relative py-2 z-10 transition-transform duration-300 ease-in-out ${selectedButton === 'signup' ? 'text-black' : 'text-white'}`}
            onClick={() => handleButtonClick('signin')}
          >
            Entrar
          </button>
          <button
            className={`relative py-2 z-10 transition-transform duration-300 ease-in-out ${selectedButton === 'signup' ? 'text-white' : 'text-black'}`}
